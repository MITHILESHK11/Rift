...
def execute_grounded_chat_query(db: Session, query: str, candidate_id: str = "") -> Dict[str, Any]:
    """Executes a Candidate-Scoped Plan -> Execute -> Phrase grounded chat pipeline."""
    ...
    if matched_assignee and not ("marketing vs" in q_lower or "proposal vs" in q_lower):
        ...
        answer = f"{name.capitalize()} (`{aid}`) has {len(tasks)} task(s) assigned for candidate `{norm_cand_id}`."
        return format_with_gemini_phraser(query, supporting_data, answer)
    ...
    if matched_cat:
        ...
        answer = f"There are {len(tasks)} task(s) in category `{matched_cat}` for candidate `{norm_cand_id}`."
        return format_with_gemini_phraser(query, supporting_data, answer)

    # 6. Specific Dynamic Keyword Search (e.g. GST refund, pet insurance, healthcare)
    # Perform strict database text search on subject / title / description
    words = [w for w in re.findall(r'\w+', q_lower) if len(w) > 2 and w not in ["how", "many", "were", "about", "emails", "tasks", "related", "the", "show"]]
    if words:
    ...
    # 7. Honest Fallback (Not guessing or returning unrelated counts)
    supporting_data = {
        "candidate_id": norm_cand_id,
        "matched_count": 0,
        "note": "No matching database query pattern or topic found for this question."
    }
    ...
    try:
        with httpx.Client(timeout=8.0) as client:
            ...
            if resp.status_code == 200:
                ...
                if text:
                    return {"answer": text, "supporting_data": supporting_data}
    except Exception:
        pass
